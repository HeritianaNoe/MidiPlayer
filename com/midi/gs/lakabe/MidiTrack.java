...
public class MidiTrack {
		...
		public static void setTempos(Sequencer sequencer, ArrayList<Long> timings, ArrayList<Float> bpms, int newTempo){
			Sequence sequence = sequencer.getSequence();
			Track track = sequence.getTracks()[0];
			for (int i = 0; i< bpms.size();i++){
				MetaMessage msg = new MetaMessage();
				long tempo = (long) (bpmTomspb(bpms.get(i) + newTempo) * 1000);
				...
				MidiEvent e = new MidiEvent(msg, timings.get(i));
				track.add(e);
			}
		}
		...
		public static byte[] tempoToDataBytes(long tempo){
			...
			byte[] data = hexStringToByteArray(tempoHexString);
			return data;
		}
		...
		public static void setEmptyTempos(Sequencer sequencer){
			Sequence sequence = sequencer.getSequence();
			...
			for (int j = 0; j< tracks.length;j++){
				...
				for (int i = 0; i < track.size();i++){
					...
					if (message instanceof MetaMessage) {
						...
						if (mm.getType() == 0x51) { // tempo message
							eventList.add(event);
						}
					}
		...
		public static void getDefaultTempos(Sequencer sequencer){
			Sequence sequence = sequencer.getSequence();
			Track[] tracks =  sequence.getTracks();
			...
			for (int j = 0; j< tracks.length;j++){
				...
				for (int i = 0; i < track.size();i++){
					...
					if (message instanceof MetaMessage) {
						MetaMessage mm = (MetaMessage) message;
						if (mm.getType() == 0x51) { // tempo message
							int t = MidiHelpers.getUnsignedInt(mm.getData(), 3);
							MidiSequencer.bpms.add(60000000.0f / t);
							MidiSequencer.timings.add(event.getTick());
						}
					}
		 ...
		 * @param volumeChange , change in volume, can be positive or negative
		 */
		public static void setTrackVolume(Sequencer sequencer, int trackID, int volumeChange) {
		...
		public static byte[] longToBytes(long x) { //64bit
		    ByteBuffer buffer = ByteBuffer.allocate(Long.BYTES);
		    buffer.putLong(x);
		    return buffer.array();
		}
		
		public static byte[] intToBytes(int x) { //32bit
		    ByteBuffer buffer = ByteBuffer.allocate(Integer.BYTES);
		    buffer.putInt(x);
		    return buffer.array();
		}
		
		public static byte[] shortToBytes(short x) {  //16bit
		...
		public static byte[] convertToBytesArray(int[] ints, ByteOrder order){
			byte[] bytes = new byte[ints.length * 4];
			ByteBuffer.wrap(bytes).order(order).asIntBuffer().put(ints);
			return bytes;
		}
		
		public static void printSequence(Sequencer sequencer, File file, int fileFormat){
			String outTxt = new String();
			Track[] tracks = sequencer.getSequence().getTracks();
			
			outTxt += new String("File format: " + fileFormat + "\n");
			outTxt += new String("Track count: " + tracks.length + "\n"); 
			
			outTxt += new String("Division: " + sequencer.getSequence().getDivisionType() + "\n"); 
			outTxt += new String("Resolution: " + sequencer.getSequence().getResolution() + "\n");
			...
			for(Track track : tracks){
				...
				for (int i = 0; i < track.size(); i++){
					...
					if(message instanceof MetaMessage){
						...
						outTxt += new String(event.getTick() + " Meta: " + Integer.toHexString(m.getStatus()) + " ! " + Integer.toHexString(m.getType()) + " ! " + Arrays.toString(m.getData()) + "\n");
					}
					else if(message instanceof ShortMessage){
						ShortMessage s = (ShortMessage)message;
						outTxt += new String(event.getTick() + " Short: " + Integer.toHexString(s.getStatus()) + " ! " + Integer.toHexString(s.getData1()) + " ! " + Integer.toHexString(s.getData2()) + "\n");
					}
					else if(message instanceof SysexMessage){
						SysexMessage x = (SysexMessage)message;
						outTxt += new String(event.getTick() + " Sysex: " + Integer.toHexString(x.getStatus()) + " ! " + Arrays.toString(x.getData()) + "\n");
					}
				}
			}
			
			String filename = file.getPath().substring(0, file.getPath().length() - 4) + ".txt";
			try {		
				Files.deleteIfExists(Paths.get(filename));
				Files.createFile(Paths.get(filename));
		        Files.write(Paths.get(filename), outTxt.getBytes(), StandardOpenOption.WRITE);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
		
		public static void saveMidi(Sequencer sequencer, File outputFile){
			try {
				MidiSystem.write(sequencer.getSequence(), 1, outputFile);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
